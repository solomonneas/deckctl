...
def test_obs_event_dataclass_holds_kind_qualifier_active():
    ev = OBSEvent(host="studio", kind="obs.recording.state", qualifier=None, active=True)
    assert ev.host == "studio"
...
def test_obs_client_constructor_does_not_connect():
    ...
    cb = MagicMock()
    c = OBSClient(host="studio", url="obsws://192.0.2.10:4455/x", on_event=cb)
    assert not c.is_connected


def test_obs_client_start_calls_event_client_constructor_with_parsed_url():
    cb = MagicMock()
    c = OBSClient(host="studio", url="obsws://192.0.2.10:4455/secret", on_event=cb)
