...
def test_check_stream_deck_passes_when_device_present():
    class _FakeDev:
        ...
        def key_count(self) -> int:
            return 15
    with patch("deckctl.doctor._enumerate_first", return_value=_FakeDev()):
        r = check_stream_deck()
    assert r.severity is Severity.PASS
...
def test_check_stream_deck_fails_when_not_found():
    with patch("deckctl.doctor._enumerate_first", return_value=None):
        r = check_stream_deck()
    assert r.severity is Severity.FAIL


def test_check_stream_deck_fails_on_probe_error():
    with patch("deckctl.doctor._enumerate_first", side_effect=RuntimeError("no HID backend")):
        r = check_stream_deck()
    assert r.severity is Severity.FAIL
...
def test_check_service_status_returns_status_severity():
    with patch("deckctl.doctor.service_status", return_value="active"):
        r = check_service_status()
    assert r.severity is Severity.PASS

    with patch("deckctl.doctor.service_status", return_value="inactive"):
        r = check_service_status()
    assert r.severity is Severity.WARN

    with patch("deckctl.doctor.service_status", return_value="failed"):
        r = check_service_status()
    assert r.severity is Severity.FAIL

    with patch("deckctl.doctor.service_status", return_value="not-installed"):
        r = check_service_status()
    assert r.severity is Severity.WARN
...
def test_check_udev_rule_pass_when_present(tmp_path: Path):
    ...
    with patch("deckctl.doctor.UDEV_RULE_PATH", fake_path):
        r = check_udev_rule()
    assert r.severity is Severity.PASS
...
def test_check_obs_reachability_warn_without_config():
    from deckctl.doctor import check_obs_reachability
...
def test_check_obs_reachability_warn_on_unreachable(tmp_path, monkeypatch):
    from deckctl.doctor import check_obs_reachability
    monkeypatch.setenv("DECKCTL_TEST_OBS_PASS", "x")
    cfg = tmp_path / "c.yaml"
    cfg.write_text(
        "version: 1\ndefault_profile: a\n"
        "obs_hosts:\n"
        "  ghost:\n    url: obsws://127.0.0.1:6666/${DECKCTL_TEST_OBS_PASS}\n"
        "profiles:\n  a:\n    default_page: home\n"
        "    pages:\n      home:\n        keys: {}\n"
    )
    r = check_obs_reachability(str(cfg))
