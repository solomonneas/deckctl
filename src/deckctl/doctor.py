"""Diagnostics for `deckctl doctor`.
...
from deckctl.service import UDEV_RULE_PATH, service_status
...
def _enumerate_first() -> Any:
    ...
    from deckctl.device.streamdeck import StreamDeckDevice
...
def check_service_status() -> CheckResult:
    ...
    if status == "failed":
        return CheckResult("service", Severity.FAIL, "systemd user unit FAILED (journalctl --user -u deckctl)")
    if status == "not-installed":
        return CheckResult("service", Severity.WARN, "service not installed (run `deckctl install-service`)")
...
def check_obs_reachability(config_path: str | None) -> CheckResult:
    ...
    from deckctl.obs.url import parse_obsws_url
    reachable: list[str] = []
