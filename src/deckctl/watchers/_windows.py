...
Untested on Linux dev - verified on real Windows by running `deckctl daemon`.
...
class WindowsForegroundWatcher:
    ...
    def start(self, on_change: WatcherCallback) -> None:
        ...
        self._cb = on_change
        self._stop.clear()
        self._thread = threading.Thread(target=self._run, name="deckctl-watcher-win", daemon=True)
        self._thread.start()
